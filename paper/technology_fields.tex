...
the model restores work but not pay: most new tasks bind to no
...
particular tasks, within particular occupations. Workers accumulate human capital in specialized
...
and the labour freed by stripping re-sorts into other occupations,
crowding them and carrying wage pressure into production and manual
work the technology never reaches. Reinstatement of work and
...
industrial-robot field in the demand channel. The robot field's centre
and reach are the Webb fit of Section~\ref{sec:automation-waves}. Its
amplitude is a modelling rule of the same kind that sets $R$:
$A_K=1.2$ opens the gate moderately in the west, and the same constants
carry through the placebo of Section~\ref{sec:directional-check}. A
calibration of the gate to the aggregate displacement of
\textcite{acemoglu2020robots}, reported in
Section~\ref{sec:directional-check}, returns $A_K=1.48$ at the 1999
price field, the same order as the rule. The
purpose is illustrative. The cases show what the geometry makes visible: where work
...
region crosses zero at $\eta\approx7.3$, the robot region at
...
the ring than to the centre. A cross-section of this kind cannot
confirm the prediction; it could have contradicted it and does not.
...
$0.63$, and reinstatement leaves it at $0.626$, because $68$ percent of
the seeded mass binds to no occupation. The wage burden splits almost
evenly between stripped bundle content ($-0.41$ log points,
employment-weighted) and the congestion that re-sorting induces
($-0.38$). The inference is bounded: the firm foundings are consistent
with the seeding prediction, the wage cross-section cannot identify the
wage prediction, and the results stand as a model-internal account the
data fail to contradict. The causal test remains within-occupation:
