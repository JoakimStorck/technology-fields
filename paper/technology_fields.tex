...
companion work. The exercise doubles as a caution for the growing
...
The implementation uses a stricter form. Over-qualification is
penalized alongside the deficit, with weight $\lambda_{\mathrm{over}}$,
and attachment decays with distance to the occupation's centroid at
scale $\rho$:
\begin{equation}
  e_o(\mathbf{r})
  =
  ...
  \label{eq:attachment}
\end{equation}
where $\bar\delta_o(\mathbf{r})=\sum_{k\in\mathcal K}
v_k\max(q_{o,k}-q_k(\mathbf{r}),0)$ is the surplus. Setting
$\lambda_{\mathrm{over}}=0$ and $\rho\to\infty$ recovers
eq.~\eqref{eq:readiness}. The stricter form keeps an over-qualified
generalist far from its core from binding seed as if it were home
territory. Total attachment capacity at
a location is
\[
  C(\mathbf{r})
  =
...
scenarios; Table~\ref{tab:economy-parameters} lists them. The
...
of order one. Mobility values are the reference at the baseline
equilibrium.}
...
$\beta$ & congestion exponent in $V(\mathbf{r})=D\,\Pi\,n^{\beta}$; human task-work is paid its marginal value product out of $\beta V$, capital is paid its cost, $(1-\beta)V$ is the congestion rent to the fixed factor \\
