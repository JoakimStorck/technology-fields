...
is an outcome. We represent a technology as a field over the
two-dimensional task space of \textcite{storck2026geometry}. Capital
...
the model restores employment but not pay: most new tasks bind to no
...
robotics firms are founded where the model places new work.
\end{abstract}
...
capital can perform the exposed work more cheaply than labour. When
automation displaces some tasks and creates others, the central question
is not only how many new tasks appear but which workers perform them:
whether the new work reaches those whose task content was displaced, and
whether performing it restores their pay. Answering it means asking where
new tasks appear, which occupations can take them, and whether the tasks
that return carry the priced content of the tasks that left.
...
seeds new work. The second concerns the
worker consequences of reinstatement: task creation restores employment
without restoring pay, and the paper traces why through a single spatial
story. New tasks strip priced content from the bundles they touch; most
of the work they seed binds to no occupation and reaches no worker; and
the labour freed by stripping re-sorts into other occupations, crowding
them and carrying wage pressure into production and manual work the
technology never reaches. Employment reinstatement and wage
reinstatement come apart, and the geometry locates where. The third is empirical.
The model's central prediction is spatial: reinstatement seeds new work
...
The same data reach the model's wage prediction and cannot identify it.
...
cases. Section~\ref{sec:directional-check} confronts the model with data of
two kinds, observed occupational wage changes and the founding locations
of new firms, and shows what each can and cannot show.
...
The model makes empirical claims of two kinds, and the available data
speak to them unevenly. One claim is about incidence, about where
automation strips paid content and pushes wages down. The other is about
location, about where reinstatement seeds new work. Observed
occupational wage changes bear on the first. The founding locations of
new firms bear on the second. We take each in turn and then say what
each can and cannot show.
...
\subsection{Where new work appears}
\label{sec:check-newwork}

The wage cross-section speaks to incidence, to where automation strips
paid content. The model makes a second prediction, about location.
...
that top to a boundary. New work should concentrate on the ring, away
from the technology centre. A scalar exposure ranking has no boundary
and no gradient, so it cannot place new work at all.

The test needs firm locations fixed without reference to where the model
seeds. We take the AI and robotics startups of
\textcite{fenoaltea2026}, tagged from the Y~Combinator directory. Each
firm's product text is embedded with the encoder of
\textcite{storck2026geometry} and mapped through the frozen projection,
which was calibrated to O*NET task statements and to wages and never to
where firms are founded. The firms enter the disk as the occupations
did. The seeding ring is a property of the calibrated field, and the
firm positions are independent of it.

The AI firms ($n=1885$) land on the ring
(Figure~\ref{fig:startup-comparison}). Their median distance to the ring
is $0.14$ in disk units, and $88$ percent sit closer to the ring than to
the technology centre. Against a uniform-disk null, seeding density at
the firm locations is enriched $1.30\times$, while the incidence field
$a(\mathbf r)$ sits at the null ($0.92\times$). The firms cluster where
the model seeds new work and away from where it strips paid content. The
robotics firms ($n=153$) sit tighter still, with $99$ percent closer to
the ring than to the centre. A cross-section of this kind can only fail
to contradict the prediction, and it does not.
...
sit on the outer lobe, off the incidence core entirely ($a$-enrichment
$0.03\times$ against $0.92\times$ for AI), where the seeded work binds to
no existing occupation. A scalar exposure ranking scores the two alike.
Pooled, the firms trace both peaks of the model's bound reinstatement.
...
projection. Right: radial density against distance from $p_K$; the
seeding ring $\|\nabla\phi_K\|$, the unbound field $u(\mathbf r)$, the
model's bound reinstatement, and the AI and robotics firms all peak near
$z_K$. The unbound share of seeded mass is $68$ percent. The firm
positions are calibrated to O*NET and wages, never to where firms are
founded.}
...
The two confrontations are not symmetric. The wage cross-section cannot
test the mechanism. The model's wage prediction and the dominant wage
shock of the period load on the same index, the baseline wage, and once
that index is held fixed no identifying variation is left. The
cross-section fails to contradict the model because it cannot
discriminate, not because it corroborates. The firm locations are
different. They bear on a spatial prediction the wage cross-section
cannot make, that new work concentrates on the ring, and they are
consistent with it. Neither reading is proof. A single cross-section of
firm positions cannot separate seeding on the ring from other processes
that would also deposit new work there. It can only fail to contradict
