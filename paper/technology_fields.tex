...
cases. Section~\ref{sec:directional-check} provides a directional
consistency check against observed occupational wage changes.
...
reduced-form field but produces too weak a north-south gradient. A
...
\]
steepens the gradient and reproduces the observed field closely. A
...
one-dimensional model would assign to labour by construction.
...
employment growth appears only at high elasticity --- the region crosses
zero between $\eta=6$ and $7$ --- or where infra-marginal savings are
large.
...
%% ============================================================
\section{Directional Consistency Check}
...
bundles move less or remain closer to high-priced work. This is not a
causal design. It is a falsification-oriented consistency check.
...
every bundle down-gradient ($96$ percent), and the prediction has the
expected sign: occupations pushed further down-gradient exhibit weaker
wage growth. The rank correlation between predicted pressure and observed
wage change is $+0.34$ across occupations ($p<10^{-20}$) and $+0.35$ at
the SOC level; the model's own bundle wage change tracks observed changes
slightly better ($+0.42$). The result is stable when May 2024 is used as
the endpoint ($+0.39$).
...
occupational wage change.}
\label{fig:centroid-shift-new}
\end{figure}

This result should be read narrowly. The test does not isolate the
causal effect of AI or any other technology. It checks whether the
spatial displacement mechanism points in the same direction as observed
relative wage movement. A model pointing the wrong way would fail this
test. Passing it supports the mechanism but does not validate its
magnitude.
