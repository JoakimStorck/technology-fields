...
robotics firms are founded where the model places new work, and in the
industrial-robot wave's own window the predicted wage pressure tracks
observed occupational wage growth independently of the wage level.
\end{abstract}
...
opposite sign. The window supports the spatial mechanism without
separating robots from the import competition in the same occupations,
so the causal test remains within-occupation.
...
automation, from a pre-technology normalization of one to $0.65$,
...
induced by inflow, stripping carries the mean: an employment-weighted
$-0.35$ log points, against a congestion mean near zero ($-0.03$).
Congestion is a distributional channel. It moves a mean absolute
...
The level is a full-maturity re-pricing rather than a forecast.
...
the software field ($-0.12$). Their correlations are the period's
...
\parencite{autor2013china}, so the window supports the spatial
mechanism without attributing it to robots against the trade shock.
...
there, the correlation survives conditioning, and the counterfactual
...
locations are different again. They bear on a spatial prediction the
wage data cannot make, that new work concentrates on the ring, and they
are consistent with it. None of the three readings is proof. A single
...
$0.65$, and reinstatement leaves it at $0.644$, because $68$ percent of
...
it without separating robots from the import competition pressing the
same occupations. The results stand as a model-internal account that
