...
fitted to occupational robot exposure in the low-priced technical-physical
...
occupations and shows the tasks behind each centroid. Within a bundle the
angle sorts tasks by kind. The Software Developers bundle runs from
consulting clients and conferring with managers on its human-centered
edge, through the analysis and system design that form its deep core, to
the shallow installation and user-training work that reaches toward the
manual west. The Lawyers bundle splits in two. An advisory arc that
interprets law, weighs precedent and counsels clients leans toward the
analytical north, and a representation arc that argues motions, presents
cases and negotiates settlements leans toward the service side. Both are
human-centered and neither is radially deep, so legal work reads as broad
judgment applied to varying cases rather than narrow specialization.
Machinists span the widest arc, and their bundle is the deepest and most
varied. It reaches from the supporting setup work on its service-leaning
edge to the conferring, testing and evaluating on its analytical edge.
Its technical core sets up and operates machine tools, measures and
diagnoses, machines and assembles parts, and in depth the bundle runs
from generic support to highly specific hands-on work. Dishwashers occupy
a short arc of a single kind, washing and cleaning, moving and stocking,
with no client contact and no analysis. Every task is a self-contained
manual operation and none is generic, bounded work whose depth the
service direction does not reward. Waiters combine two kinds in one job,
the preparation and clearing that leans toward the service-technical side
and the customer interaction that leans toward the human-centered side,
with the interaction tasks the more specific of the two.
...
\begin{table}[!htbp]
\centering
\caption{Technology calibration. The cognitive field is fitted to task-level AI exposure by least squares. Fit is the $R^2$ on the smoothable between-cell exposure surface. The industrial-robot field, the second technology carried through the analysis, is calibrated to Webb's robot exposure in Section~\ref{sec:automation-waves}.}
...
We run the cognitive field through the calibrated economy, and add the
industrial-robot field in the demand channel. The purpose is
illustrative. The cases show what the geometry makes visible: where work
is taken, where reinstatement is seeded, which occupations can bind it,
how much remains unbound, and how demand elasticity changes employment
consequences.
...
$22$ percent for the cognitive field and $9$ percent for the robot one.
...
region crosses zero at $\eta\approx7.3$ and the robot region at
$\eta\approx7.4$ --- or where infra-marginal savings are large.
...
industrial-robot field of Section~\ref{sec:automation-waves}, a real
...
technical-physical arc, where the price field is low and no occupation
holds the priced capabilities the seeded tasks require. The model
therefore predicts that robot reinstatement binds to no occupation and
returns no offsetting employment, the geometric form of the absence of
positive employment effects outside the exposed region documented by
\textcite{acemoglu2020robots}.
...
of Section~\ref{sec:automation-waves}, a technology whose deployment
