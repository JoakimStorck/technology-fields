...
the cross-section, validated against institutional composition, and
held fixed under technology shocks. The wedge is not part of the
theory; simulations are reported with and without it
...
A new task at $\mathbf{r}$ requires a profile of capabilities. We carry
the requirement as four fields over the disk, one per capability
cluster of \textcite{storck2026geometry}, of the same functional family
as the price field: first-harmonic in direction, linear in depth,
\begin{equation}
  q_k(\xi, \chi)
  \;=\;
  \alpha_{k,0} + \alpha_{k,1}\cos\xi + \alpha_{k,2}\sin\xi
  \;+\; \chi\,\bigl(\alpha_{k,3} + \alpha_{k,4}\cos\xi
  + \alpha_{k,5}\sin\xi\bigr),
  \label{eq:capability-field}
\end{equation}
for $k \in \{\mathrm{S1}, \mathrm{S2}, \mathrm{A1}, \mathrm{A2}\}$,
fitted to occupation-level cluster intensities
(Section~\ref{sec:calibration}). This is the minimal parametrization of
the two regularities the geometry establishes: the direction $\xi$ sets
which capabilities the work demands, and the level demanded rises with
$\chi$. The fields read the requirement at a location from the
...
near zero for the ability clusters A1 and A2. They are measured there
and held fixed here, inputs rather than free parameters
(Section~\ref{sec:calibration}). A single set of weights
...
\textcite{gathmann2010}. The allocation is a logit
