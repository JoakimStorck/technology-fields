...
with the tempo. This path-dependence of the
...
reallocation of workers run in parallel, not in sequence. The
task-based framework cannot see this. Where it has dynamics they belong
...
(Section~\ref{sec:homotopy}). The destination, not just
...
workers waiting for work. The unbound stock of this paper is the reverse
...
different occupations. The calibration remains illustrative, in the
...
downgraded. The leading gainers make the point concretely: in the
...
locate this crossover between two allocations that are derived, not
simulated.
...
\textcite{storck2026fields}, Section~3.3, and an equilibrium that does
not equilibrate discards its own content. \textcite{acemoglu2024rents} make the point sharply: in their
...
headline is not robust to the sign, which therefore must
...
not. That extension is left for later work. New tasks enter as added
