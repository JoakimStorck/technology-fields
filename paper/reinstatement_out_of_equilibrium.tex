...
allocations of the slow and fast regimes correlate at $0.37$. The two regimes are the proved limits of
...
fate $s\,a$. The generalisation is what opens the destination question.
...
captures a part $s\,a$, occupations bind a part $s(1-a)\Phi$, and a
part $s(1-a)(1-\Phi)$ remains unbound, where $s = M\hat g$ and $\Phi$
is the bound share. The unbound part performs no work and does not enter the
...
size-limited capacity $c_o = M_o/\theta$ is small, the mass accumulates
...
range while its point size depends on the handle. Decomposing the tempo
into its two timescales shows which one binds. Slowing only absorption
...
dynamic end-state price level ($-0.285$) and labour share ($0.739$)
reproduce the static fixed point ($-0.26$, $0.73$) to within rounding.
Two independent constructions, static fixed-point iteration and dynamic
time-stepping, reach the same equilibrium, so the price feedback is a
property of the model rather than of either method, and the static and
dynamic accounts cohere.
