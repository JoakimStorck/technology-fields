...
A technological breakthrough can happen overnight, but its consequences 
cannot be overviewd from the start. It unfolds over the coming months and years, 
and while it does, the labour market is already responding. 
Investments are made, new work practice starts to spread,
training and education equips engineers and workers with the required skills.
Educational programmes introduce students to the new knowledge.
As the technology emerges, diffuses, and matures, old tasks are displaced, 
new tasks are introduced, and workers notice the change. Some find themselves doing new things, 
others are reallocated. These processes run in parallel. 

The spread and diffusion of technological change has been studied by...
Some important traditions are...
The task based tradition (refs) has shown interesting results...

The task-based framework cannot see this. Where it has dynamics they belong
to the technology --- which tasks are automated, when new ones arrive
\parencite{acemoglu2018race} --- while the allocation of workers to
tasks clears instantaneously, so the consequences of a technology are
read as the difference between two rest points
\parencite{acemoglu2019automation}. The intermediate path of the
allocation, and with it the cost of the transition and any dependence of
the outcome on the speed of the shock, is by construction absent. Yet the transition is where the human consequences of automation
...
\textcite{storck2026geometry} construct the task space: tasks and
occupations are mapped from O*NET statements onto a two-dimensional
disk, occupations as bundles of tasks. \textcite{storck2026fields}
builds the model of technological change on that geometry: the market
price of skill is a directional field over the space; a technology is a
Gaussian augmentation field; automation removes the high-priced tasks of
a bundle; and reinstatement seeds new tasks on the gradient boundary of
the technology field, which survive as human work only where labour
remains the cheaper producer. That model is closed as a static
equilibrium, with the adjustment dynamics left explicitly for later
work; the rise-and-fall of employment which
\textcite{bessen2019demand} documents over time appears in it only as a
cross-section over the demand elasticity, not as a trajectory. This
paper is that later work.

We make one structural addition and otherwise reuse the static model
unchanged. That model creates new task mass on the boundary ring
...
of reinstated work correlate at $0.37$. Both endpoints are proved as
limits of the binding law --- absorption faster than the shock converges
to the match-weighted allocation, absorption slower than the shock to
initial size shares --- and the realised destination moves monotonically
between them as the tempo varies
...
occupation --- mismatch upstream of any vacancy --- and its hump is the
model's analogue of their cyclical index, driven by task destruction and
...
and demand is met by the entry of new generations. In their model the
...
slowly or quickly through the same economy, binds the reinstated work to
different occupations. The tempo is also a policy margin:
...
The calibration remains illustrative, in the
discipline of \textcite{storck2026fields}: no causal claim is made from
it.
