...
\begin{abstract}
\noindent
When automation creates new tasks, which workers perform them? Task-based
models fix the answer by construction; here occupations are task bundles
and the destination of new work is an outcome. We represent an automation
technology as a field over a two-dimensional task space built from O*NET
tasks. Capital takes a task where it is cheaper; new work becomes human
only where labour stays competitive and an occupation binds it. Two waves
run in their own eras: industrial robots in the 1999 economy, a cognitive
field calibrated to AI exposure. Both restore work but not pay: most
seeded tasks bind to no occupation and the labour share does not recover.
Bound new work lands at nearly one price under both; field position sets
the price of what each takes. New AI and robotics firms fall on opposite
flanks of the model's seeding region, as their field positions imply,
apart from a control corpus of new firms. The recent wage cross-section
cannot identify the model's wage prediction; placebo technologies,
including one that does not exist, clear the same check. In the robot
wave's own window predicted pressure does track occupational wage growth,
independently of wage level and manufacturing share.
\end{abstract}
...
each, and it stays the majority of the seed at every tested scale of the
shock: a half-point swing under the robot field between half and double
the calibrated moment, a $53$-to-$79$ percent range under the cognitive
rental sweep (Section~\ref{sec:cognitive-case}). Neither is a property
of where the cognitive field happens to sit. What position governs is
the price of the work each wave destroys, which is the last three rows
of the table.
...
a majority of the seed stays unbound at every tested scale of the shock,
with a half-point swing under the robot field and a $53$-to-$79$ percent
range under the cognitive rental sweep. Neither is a
