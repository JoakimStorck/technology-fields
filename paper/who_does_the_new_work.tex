...
The effects of automation on work content and jobs is of interest not 
least with the advent of AI-automation technologies with the potential of automating
intellectual work, with its effects already affecting the labour market.
Historically, the introduction of new automation technologies has resulted in
manual tasks becoming obsolete while new tasks have been introduced. 
Work is reshaped: Some jobs disappear while new jobs emerge. 
The work content of occupations change, and so does the qualifications for holding an occupation.
Workers are often displaced, and new jobs are created.
So who does the new work, and is it possible to say something about the nature of the new work that emerge?

This paper introduces a theory of automation that builds on and extends existing task based models.
Automation acts on particular tasks, within particular occupations. 
Workers accumulate human capital in specialized domains, occupations combine multiple tasks into bundles, and
technologies vary in where they act, how far they reach, and whether
capital can perform the exposed work more cheaply than labour. 
When automation displaces some tasks and creates others, the central question
...
Assume that work of all kinds can be sorted according to task content into 
a two dimensional map where all occupations are positioned according to the type of work they represent.
Then assume that any automation technology can be represented as an operator
that makes capital productive at performing those kinds of task content that are suitable for that particular automation technology. 

In the following, we show that these assumptions not only are viable but have inteeresting outcomes.
We introduce the concept of a technology field: a
...
structure as a price of work, a field over the space, and gives
technologies a location in the same space as the work they act on; the
consequences
...
But at the adoption margin the saving is second-order, so large
employment gains require elastic demand or large infra-marginal savings
deep inside the technology's core. This is the spatial form of the
demand mechanism of \textcite{bessen2019demand} and the second-order
margin of \textcite{acemoglu2018race}.
...
calibration and validates the seeding prediction against firm
locations. Section~\ref{sec:cases} presents the illustrative cases.
