#!/usr/bin/env python3
"""
Extract a compact causal-structure graph from the dynamic model code.

The script produces three layers:
...
The extraction is deliberately conservative. Manual rules in concept_map.toml
carry the economic interpretation and the sign of key feedback relations.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--source",
        default="experiment/run_dynamic.py",
        help="Path to the dynamic model Python file.",
    )
    parser.add_argument(
        "--map",
        default="experiment/cld/concept_map.toml",
